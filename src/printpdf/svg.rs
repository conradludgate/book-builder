...
use crate::printpdf::{Pt, Px, XObject, XObjectRef};
use lopdf::{Object, Stream};
use std::{error, fmt};

use super::{PdfLayer, PdfLayerIndex, PdfPage};
...
impl SvgXObjectRef {
    pub fn add_to_layer(self, layer: &mut PdfLayer, transform: SvgTransform) {
        use crate::printpdf::CurTransMat;
...
impl Svg {
    ...
    /// than once on a page
    pub fn into_xobject(self, page: &mut PdfPage) -> SvgXObjectRef {
        let width = self.width;
        let height = self.height;
        let xobject_ref = page.add_xobject(XObject::External(self.svg_xobject));
    ...
    pub fn add_to_layer(self, page: &mut PdfPage, layer: PdfLayerIndex, transform: SvgTransform) {
        let xobject = self.into_xobject(page);
        xobject.add_to_layer(page.get_layer(layer), transform);
    }
}
